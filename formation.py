...
from mip import Model, xsum, BINARY, minimize, GUROBI
...
max_skill_level = sum(skill_levels) // len(teams)  # Ideal max skill per team
...
for t in teams:
    team_skill = xsum(skill_levels[c] * x[c][t] for c in campers)
    m += imbalance[t] >= team_skill - max_skill_level
    m += imbalance[t] >= max_skill_level - team_skill
...
# Print Results
for t in teams:
    print(f"Team {t+1}: ", [c for c in campers if x[c][t].x >= 0.99])
